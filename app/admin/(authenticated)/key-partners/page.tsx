...
import { keyPartnersService } from '@/lib/services'
import type { KeyPartner, KeyPartnerMember } from '@/lib/services'
...
export default function KeyPartnersPage() {
  const [partners, setPartners] = useState<KeyPartner[]>([])
  ...
  const fetchPartners = useCallback(async () => {
    setLoading(true)
    try {
      setPartners(await keyPartnersService.getAll())
    } catch {
      showToast('error', 'Gagal memuat key partners')
    } finally {
      setLoading(false)
    }
  }, [])
