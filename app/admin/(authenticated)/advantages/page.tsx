        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10, padding: '14px 24px', borderTop: `1px solid ${theme.divider}` }}>
            ...
            <button type="submit" disabled={saving}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 20px', borderRadius: 12, fontSize: 13, fontWeight: 600, color: '#fff', border: 'none', cursor: 'pointer', transition: 'all 0.15s', background: saving ? 'rgba(7,82,183,0.5)' : theme.accent, boxShadow: saving ? 'none' : '0 2px 12px rgba(7,82,183,0.25)' }}>
              {saving
        ...
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '14px 20px', borderTop: `1px solid ${theme.divider}` }}>
            <button onClick={handleSave} disabled={saving || uploading || !canEdit}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 20px', borderRadius: 12, fontSize: 13, fontWeight: 600, color: '#fff', border: 'none', cursor: 'pointer', transition: 'all 0.15s', background: (saving || uploading || !canEdit) ? 'rgba(7,82,183,0.5)' : theme.accent, boxShadow: (saving || uploading || !canEdit) ? 'none' : '0 2px 12px rgba(7,82,183,0.25)' }}>
              {saving
                ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full admin-spin" />Menyimpan...</>
                : <><span className="material-symbols-outlined" style={{ fontSize: 15 }}>save</span>Simpan</>}
            </button>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          {edit && (
            <button onClick={() => setModal({ mode: 'add', advantage: { order: nextOrder } })}
              ...
              <span className="material-symbols-outlined" style={{ fontSize: 15 }}>add</span><span className="hidden sm:inline">Tambah Advantage</span>
            </button>
          )}
        </div>
      </div>
